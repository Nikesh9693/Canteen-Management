  ...
  class signUpMethods {
    ...
    static authRedirecting() {
      window.setTimeout(() => {
        window.location.replace('https://rvscollege.ac.in/client-side');
      }, 500)
    }
    ...
    firebaseAuthRedirect(){
      firebase.auth().onAuthStateChanged(function(user) {
        ...
        user ?  window.location.replace('https://rvscollege.ac.in/client-side') : console.log('none');
      });
    }
  }
      ...
      logout.forEach(btn => {
        btn.addEventListener('click', (e) => {
          ...
          firebase.auth().signOut().then(() => {
            window.location.replace("https://rvscollege.ac.in/index.html")
          });
        });
      })
    }
